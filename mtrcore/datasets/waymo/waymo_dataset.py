...
from mtrcore.datasets.dataset import DatasetTemplate
...
class WaymoDataset(DatasetTemplate):
    ...
    def create_agent_data_for_center_objects(
            self, center_objects, 
            obj_trajs_past, obj_trajs_future, 
        ...
        Args:
            ...
            ...
        Returns:
            obj_trajs_data (num_center_objects, num_objects, num_timestamps, num_attrs = 29): ---> self.generate_centered_trajs_for_agents() ---> ret_obj_trajs = torch.cat((
            obj_trajs_mask
            obj_trajs_pos
            obj_trajs_last_pos
            obj_trajs_future_state
    ...
    def transform_trajs_to_center_coords(obj_trajs, center_xyz, center_heading, heading_index, rot_vel_index=None):
        """
        Args:
            obj_trajs (num_objects, num_timestamps, num_attrs): first three values of num_attrs are [x, y, z] or [x, y]
    ...
    def generate_centered_trajs_for_agents(self, center_objects, obj_trajs_past, obj_types, center_indices, sdc_index, timestamps, obj_trajs_future):
        ...
        vel = obj_trajs[:, :, :, 7:9]                   # (num_center_objects, num_objects, num_timestamps, 2)
        vel_pre = torch.roll(vel, shifts=1, dims=2)
        acce = (vel - vel_pre) / 0.1                    # (num_center_objects, num_objects, num_timestamps, 2)
        acce[:, :, 0, :] = acce[:, :, 1, :]

        ret_obj_trajs = torch.cat((
            obj_trajs[:, :, :, 0:6],                    # (num_center_objects, num_objects, num_timestamps, 6)                      [cx, cy, cz, dx, dy, dz]
            object_onehot_mask,                         # (num_center_objects, num_objects, num_timestamps, 5)
            object_time_embedding,                      # (num_center_objects, num_objects, num_timestamps, num_timestamps+1)       num_timestamps=11
            object_heading_embedding,                   # (num_center_objects, num_objects, num_timestamps, 2)                      [np.sin(heading), np.cos(heading)]
            obj_trajs[:, :, :, 7:9],                    # (num_center_objects, num_objects, num_timestamps, 2)                      [vel_x, vel_y]
            acce,                                       # (num_center_objects, num_objects, num_timestamps, 2)
        ), dim=-1)
    ...
    def create_map_data_for_center_objects(self, center_objects, map_infos, center_offset):
        ...
        def transform_to_center_coordinates(
            neighboring_polylines,
            neighboring_polyline_valid_mask
        ):
