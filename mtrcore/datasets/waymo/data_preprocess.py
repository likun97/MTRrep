...
def get_infos_from_protos(data_path, output_path=None, num_workers=8):
    ...
    # # https://github.com/sshaoshuai/MTR/issues/8
...
def create_infos_from_protos(raw_data_path, output_path, num_workers=4):
    ...
    train_20s_infos = get_infos_from_protos(
        data_path=os.path.join(raw_data_path, 'training_20s'),
    ...
    with open(train_filename, 'wb') as f:
        pickle.dump(train_20s_infos, f)
    print('----------------Waymo info train_20s file is saved to %s----------------' % train_filename)
