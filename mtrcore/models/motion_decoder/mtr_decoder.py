...
from mtrcore.models.utils.transformer import transformer_decoder_layer
from mtrcore.models.utils.transformer import position_encoding_utils
from mtrcore.models.utils import common_layers
from mtrcore.utils import common_utils, loss_utils, motion_utils
from mtrcore.config import cfg
...
class MTRDecoder(nn.Module):
    ...
    def __init__(self, in_channels, config):
        ...
        self.intention_points, self.intention_query, self.intention_query_mlps = self.build_motion_query(
            self.d_model,
            use_place_holder = self.use_place_holder
        )
    ...
    # for above self. in __init__()
    def build_transformer_decoder(
        self,
        in_channels, d_model, nhead,
        dropout=0.1, num_decoder_layers=1, use_local_attn=False
    ):
    ...
    # for above self. in __init__(); Load intention_points .pkl file
    ...
    # in flowwing Func --> apply_transformer_decoder() >>> self.intention_points {Load intention_points .pkl file}<<<
    def get_motion_query(self, center_objects_type):
        num_center_objects = len(center_objects_type)
        if self.use_place_holder:
            raise NotImplementedError
        else:
            # intention_points .pkl file
            intention_points = torch.stack(
                [
                    self.intention_points[
                        center_objects_type[obj_idx]
                    ]
                    for obj_idx in range(num_center_objects)
                ],
                dim=0
            ) # (num_center_objects, num_query=64, 2)
            intention_points = intention_points.permute(1, 0, 2)  # (num_query=64, num_center_objects, 2)

            intention_query = position_encoding_utils.gen_sineembed_for_position(intention_points, hidden_dim = self.d_model)  # (num_query=64, num_center_objects, C=512) “DAB-DETR/”
            intention_query = self.intention_query_mlps( intention_query.view(-1, self.d_model) ).view(-1, num_center_objects, self.d_model)  # (num_query, num_center_objects, C=512)
        
        print("get_motion_query", intention_query.shape); import pdb; pdb.set_trace()
        return intention_query, intention_points


    # in flowwing Func --> apply_transformer_decoder()
    def apply_cross_attention(
        ...
        attention_layer,# self.build_transformer_decoder = class TransformerDecoderLayer() [SA CA]
        dynamic_query_center = None, # dynamic_query_center
        layer_idx = 0,
        # Paras for Query map(not object) feature
        use_local_attn = False,
        query_index_pair = None, query_content_pre_mlp = None, query_embed_pre_mlp = None
    ):
        """ Args:
            kv_feature (B, N, C):
            kv_mask (B, N):
        ...
        Returns:
            ...
            attn_weights:
        """ 
        # 1.Query object feature  
            # kv_feature,     = obj_feature     [7, 17, 512]
            # kv_mask,        = obj_mask        [7, 17 ],  
            # kv_pos,         = obj_pos         [7, 17, 3]
            # query_content,  = query_content   [64, 7, 512],
            # query_embed,    = intention_query [64, 7, 512]
            # dynamic_query_center = intention_points  [64, 7, 512]
            # attention_layer, self.build_transformer_decoder = class TransformerDecoderLayer() [SA CA]
        ...
        # 2.Query map feature  
            ...
            # query_content,  = query_content           [64,  7, 512],
            # query_embed,    = intention_query         [64,  7, 512]
            # dynamic_query_center = intention_points   [64,  7,   2]
            # attention_layer, self.build_transformer_decoder = class TransformerDecoderLayer() [SA CA]
            # use_local_attn
        ...
        num_q, batch_size, d_model = query_content.shape # 64 bs 512
        # dynamic_query_center-->searching_query
        ...
        # Query map feature
            ...
            batch_size, num_kv, _ = kv_feature.shape    # (Ni, Nm=768, Cm=256)  batch_size = Ni
            kv_feature_stack      = kv_feature.flatten(start_dim=0, end_dim=1)  # (Ni*Nm, Cm=256)
            kv_pos_embed_stack    = kv_pos_embed.permute(1, 0, 2).contiguous().flatten(start_dim=0, end_dim=1)  # (Nm=768, Ni, Cm=256)
            kv_mask_stack         = kv_mask.view(-1)    # (Ni*Nm)

            query_index_pair = query_index_pair.view(batch_size * num_q, -1)    # (B*Nq, Cm+128=384)
            key_batch_cnt    = num_kv * torch.ones(batch_size).int().to(kv_feature.device)  # (batch_size) -> value = num_kv = Nm
            index_pair_batch = torch.arange(batch_size).type_as(key_batch_cnt)[:, None].repeat(1, num_q).view(-1)  # (B*Nq)
    ...
    def apply_dynamic_map_collection(
        self,
        map_pos, map_mask, pred_waypoints, base_region_offset, num_query,
        ...
        import pdb; pdb.set_trace()
        dynamic_dist = (pred_waypoints[:, :, None, :, 0:2] - map_pos[:, None, :, None, 0:2]).norm(dim=-1) # (num_center_objects, num_query, num_polylines, 1)
    ...
    def apply_dense_future_prediction(self, obj_feature, obj_mask, obj_pos):
        ...
        obj_fused_feature_valid = torch.cat((obj_pos_feature_valid, obj_feature_valid), dim=-1)                         # torch.cat() 

        pred_dense_trajs_valid = self.dense_future_head(obj_fused_feature_valid)                                        # self.dense_future_head()
        ...
        pred_dense_trajs_valid = torch.cat((temp_center, pred_dense_trajs_valid[:, :, 2:]), dim=-1)                     # torch.cat() 
        ...
        obj_future_feature_valid = self.future_traj_mlps(obj_future_input_valid)                                        # self.future_traj_mlps()

        obj_full_trajs_feature = torch.cat((obj_feature_valid, obj_future_feature_valid), dim=-1)                       # torch.cat() 
        obj_feature_valid = self.traj_fusion_mlps(obj_full_trajs_feature)                                               # self.traj_fusion_mlps()

        ret_obj_feature = torch.zeros_like(obj_feature)
        ret_obj_feature[obj_mask] = obj_feature_valid

        # 用于get_dense_future_prediction_loss()
        ...
        return ret_obj_feature, ret_pred_dense_future_trajs # ret--> return
    ...
    def apply_transformer_decoder(
        ...
        center_objects_type, 
        obj_feature, obj_mask, obj_pos, # [7, 17,   512],  [7, 17 ],  [7, 17,   3]
        map_feature, map_mask, map_pos  # [7, 768,  256],  [7, 768],  [7, 768,  3]
    ):
        """
        对比之前Transformer Decoder, 输入不同在于query来源: 除了自身输入是 query_content feature C_j(KxD), 还多了Motion Query Pair(I(Kx2), Y_T_j(Kx2))

        输出 len(pred_list) = 6  each num_decoder_layers
                pred_list[0] = [pred_scores, pred_trajs]
                    pred_list[0][0].shape = pred_scores = (num_center_objects, num_query)
                    pred_list[0][1].shape = pred_trajs  = (num_center_objects, num_query, 80, 7)

        论文图1, 迭代: 对应知乎解释和下面代码: 
            intention_points    (num_query, num_center_objects, 2)      torch.Size([64, 7, 2])      >>  I(Kx2)        load pkl file而来, 每个centerObj离线K-means得到K条备选num_query=64个终点
            intention_query     (num_query, num_center_objects, D)      torch.Size([64, 7, 512])                    = MLP(Gen_sineembed_for_position(intention_points))
            query_content       (num_query, num_center_objects, D)      torch.Size([64, 7, 512])    >>  Cj(KxD)     = Zeros_like(intention_query);   decoder输出

            query_position      pred_waypoints, dynamic_query_center    --->    Motion Query Pair( I(Kx2), Y_T_j(Kx2))  ---> 初始化为预定义的intention_points

            num_query = 64; 理解为 当前obj_feature[7, 17, 512]中7辆中心车17辆他车 最多预测出64条 ???
            C = self.d_model = 512
        ...
        dynamic_query_center = intention_points                            # [MotionQueryPair-> Y_T_j(Kx2), Init as I] ; (num_query=64,, num_center_objects, 2)   

        num_center_objects = query_content.shape[1]
        num_query          = query_content.shape[0]
        center_objects_feature = center_objects_feature[None, :, :].repeat(num_query, 1, 1)  # (num_query=64, num_center_objects, C=512)
        import pdb; pdb.set_trace()
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            # Query object feature  
            """ query_content,  pred_waypoints=I,  dynamic_query_center;   TransformerDecoderLayer(nn.Module) [SA CA]"""                                        
            obj_query_feature = self.apply_cross_attention(
                ...
                query_content        = query_content,
                query_embed          = intention_query,
                attention_layer      = self.obj_decoder_layers[layer_idx], # = self.build_transformer_decoder() = class TransformerDecoderLayer(nn.Module) [SA CA]
                dynamic_query_center = dynamic_query_center,
                layer_idx = layer_idx
            )
                # obj_feature, obj_mask, obj_pos: [Ni, Nobj, C=512],  [Ni, Nobj ],  [Ni, Nobj, 3]
                # query_content, intention_query, dynamic_query_center: [Nq, Ni, C=512], [Nq, Ni, C=512], [Nq, Ni, 2]

            # pre-operation for query map feature
            ...
            map_query_feature = self.apply_cross_attention(
                ...
                query_content    = query_content,
                query_embed      = intention_query,
                attention_layer  = self.map_decoder_layers[layer_idx], # = self.build_transformer_decoder() = class TransformerDecoderLayer(nn.Module) [SA CA]
                layer_idx        = layer_idx,
                dynamic_query_center = dynamic_query_center,
                # more than above "Query object feature"
                use_local_attn   = True,
            ...
            # 得到C_A_j 和 C_M_j 之后，cat->MLP融合得到 C_j+1, 作为j+1层decoder的输入query_content
            query_feature = torch.cat([center_objects_feature, obj_query_feature, map_query_feature], dim=-1) # [Nq, Ni, 1280] = [Nq, Ni, C=512]+[Nq, Ni, C=512]+[Nq, Ni, C=256] 
            ...
            query_content_t = query_content.permute(1, 0, 2).contiguous().view(num_center_objects * num_query, -1) # [Nq*Ni, 512]  
            pred_scores     = self.motion_cls_heads[layer_idx](query_content_t).view(num_center_objects, num_query) # [Nq*Ni, 1] -> [Ni, Nq] 

            if self.motion_vel_heads is not None: # 如果给速度head
                ...
                pred_trajs = self.motion_reg_heads[layer_idx](query_content_t).view(num_center_objects, num_query, self.num_future_frames, 7) # [Nq*Ni, 560] -> [Ni, Nq, 80, 7] 
    ...
    def forward(self, batch_dict):
        ...
        obj_feature, obj_mask, obj_pos = batch_dict['obj_feature'], batch_dict['obj_mask'], batch_dict['obj_pos']   # torch.Size([7, 17,  256]), torch.Size([7, 17]),  torch.Size([7, 17, 3]))
        map_feature, map_mask, map_pos = batch_dict['map_feature'], batch_dict['map_mask'], batch_dict['map_pos']   # torch.Size([7, 768, 256]), torch.Size([7, 768]), torch.Size([7, 768, 3]))
        center_objects_feature         = batch_dict['center_objects_feature']                                       # torch.Size([7, 256])

        num_center_objects, num_objects, _ = obj_feature.shape
        num_polylines = map_feature.shape[1]

        # input projection 256->512 (D_MODEL=512)
        center_objects_feature = self.in_proj_center_obj(center_objects_feature)                                    # torch.Size([7, 512])

        # input projection 256->512 then filter_valid
        ...
        obj_feature[obj_mask] = obj_feature_valid                                                                   # torch.Size([7, 17,  512]),
        ...
        map_feature[map_mask] = map_feature_valid                                                                   # torch.Size([7, 768,  256]),
        ...
        )                                                                                                           # 根据agent过去的特征A_past预测未来轨迹,编码为特征A_future; 拼接[A_past, A_future]得到融合特征返回
    ...
    def get_decoder_loss(self, tb_pre_tag=''):
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            loss_cls = F.cross_entropy(
                input = pred_scores, target = center_gt_positive_idx, reduction = 'none'
            )
            # total loss
        ...
        print("get_decoder_loss", '\n', loss_reg_gmm, '\n', loss_reg_vel, '\n', loss_cls); #import pdb; pdb.set_trace()
    ...
    def get_loss(self, tb_pre_tag = ''):
        loss_decoder, tb_dict, disp_dict = self.get_decoder_loss(
